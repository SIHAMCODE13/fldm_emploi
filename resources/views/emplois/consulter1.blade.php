...
<div class="container py-4">
    <div class="card shadow">
        ...
        <div class="card-body">
            <form action="{{ route('emplois.rechercher1') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="filiere" name="filiere" required>
                            ...
                            @foreach($filieres as $filiere)
                                <option value="{{ $filiere->id_filiere }}" {{ old('filiere', $filiere_id ?? '') == $filiere->id_filiere ? 'selected' : '' }}>{{ $filiere->nom_filiere }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadGroups(filiereId) {
        ...
        if (filiereId) {
            ...
            groupeSelect.appendChild(loadingOption);
            ...
            fetch(`/api/groupes?filiere_id=${filiereId}`)
                ...
                .then(data => {
                    ...
                    data.forEach(groupe => {
                        ...
                        // Sélectionner l'option si elle correspond à l'ancienne valeur
                        if (groupe.id_groupe == '{{ old("groupe") }}') {
    ...
    // Charger les groupes pour la filière sélectionnée au chargement de la page
    const initialFiliereId = filiereSelect.value;
    if (initialFiliereId) {
        loadGroups(initialFiliereId);
    }

    // Écouter les changements sur la sélection de filière
    filiereSelect.addEventListener('change', function() {
        const filiereId = this.value;
        loadGroups(filiereId);
    });
});
</script>
@endsection