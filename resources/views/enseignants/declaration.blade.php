...
<head>
  ...
  <title>Déclaration de Non-Disponibilité - Enseignant</title>
...
<div id="content">
  ...
  <div class="welcome-box">
    <h4>Déclaration de non-disponibilité, <span>{{ auth()->user()->name }}</span> </h4>
  </div>

  <div class="form-container">
    <h2 class="form-title">Déclaration de Non-Disponibilité</h2>
    
    <form action="{{ route('enseignant.declaration.store') }}" method="POST" id="declarationForm">
      @csrf
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <select class="form-select" id="type_periode" name="type_periode" required>
            ...
            <option value="journee" {{ old('type_periode') == 'journee' ? 'selected' : '' }}>Journée complète</option>
            <option value="periode" {{ old('type_periode') == 'periode' ? 'selected' : '' }}>Période spécifique</option>
          </select>
        </div>
      </div>
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-6">
          ...
          <input type="date" class="form-control" id="date_fin" name="date_fin" 
                 value="{{ old('date_fin') }}" min="{{ date('Y-m-d') }}">
      ...
      <div id="journee_info" class="alert alert-info mt-3" style="display: none;">
        <i class="fas fa-info-circle me-2"></i> Pour une journée complète, vous serez indisponible toute la journée.
      </div>
      ...
      <div class="mb-4">
        ...
        <textarea class="form-control" id="raison" name="raison" rows="4" 
                  ...
                  required maxlength="500">{{ old('raison') }}</textarea>
        <div class="form-text"><span id="charCount">0</span>/500 caractères</div>
      </div>
      ...
      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        ...
        <button type="submit" class="btn btn-primary">
          <i class="fas fa-paper-plane me-1"></i> Soumettre la déclaration
        </button>
      </div>
    </form>
  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Logique spécifique à la déclaration
    ...
    const journeeInfo = document.getElementById('journee_info');
    ...
    function togglePeriodeFields() {
      if (typePeriode.value === 'periode') {
        ...
        journeeInfo.style.display = 'none';
        periodeSelect.setAttribute('required', 'required');
      } else if (typePeriode.value === 'journee') {
        periodeField.style.display = 'none';
        journeeInfo.style.display = 'block';
    ...
    if (typePeriode) {
      typePeriode.addEventListener('change', togglePeriodeFields);
      
      // Initialiser l'affichage basé sur les anciennes valeurs
      togglePeriodeFields();
    }
    ...
    if (raisonTextarea) {
      ...
      charCount.textContent = raisonTextarea.value.length;
    }
