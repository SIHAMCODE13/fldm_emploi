...
<head>
  ...
  <title>Modifier déclaration - FLDM</title>
...
<div id="sidebar">
  ...
  <a href="{{ route('enseignant.dashboard') }}" class="sidebar-item {{ request()->routeIs('enseignant.dashboard') ? 'active' : '' }}">
    <i class="fas fa-home"></i> <span>Home</span>
  </a>
  <a href="{{ route('enseignant.emplois-temps') }}" class="sidebar-item {{ request()->routeIs('enseignant.emplois-temps') ? 'active' : '' }}">
    <i class="fas fa-calendar-alt"></i> <span>Emplois de temps</span>
  </a>
...
<div id="content">
  ...
  <div class="welcome-box">
    <h4>Modifier votre déclaration, <span>{{ auth()->user()->name }}</span> </h4>
  </div>

  <div class="form-container">
    <h2 class="form-title">Modifier la déclaration de non-disponibilité</h2>
    
    <form action="{{ route('enseignant.declaration.update', $declaration->id) }}" method="POST" id="editForm">
      @csrf
      @method('PUT')
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <select class="form-select" id="type_periode" name="type_periode" required>
            ...
            <option value="journee" {{ old('type_periode', $declaration->type_periode) == 'journee' ? 'selected' : '' }}>Journée complète</option>
            <option value="periode" {{ old('type_periode', $declaration->type_periode) == 'periode' ? 'selected' : '' }}>Période spécifique</option>
          </select>
        </div>
      </div>
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="date" class="form-control" id="date_debut" name="date_debut" 
                 value="{{ old('date_debut', $declaration->date_debut->format('Y-m-d')) }}" required>
        </div>
        <div class="col-md-6">
          <label for="date_fin" class="form-label">Date de fin</label>
          <input type="date" class="form-control" id="date_fin" name="date_fin" 
                 value="{{ old('date_fin', $declaration->date_fin ? $declaration->date_fin->format('Y-m-d') : '') }}">
          <div class="form-text">Laissez vide si c'est pour un seul jour</div>
        </div>
      </div>

      <div class="row mb-3 periode-fields" id="periode_field">
        <div class="col-md-6">
          <label for="periode" class="form-label">Période *</label>
          <select class="form-select" id="periode" name="periode">
            <option value="">Sélectionnez la période</option>
            <option value="8:30h-10:30h" {{ old('periode', $declaration->periode) == '8:30h-10:30h' ? 'selected' : '' }}>8:30h - 10:30h</option>
            <option value="10:30h-12:30h" {{ old('periode', $declaration->periode) == '10:30h-12:30h' ? 'selected' : '' }}>10:30h - 12:30h</option>
            <option value="14:30h-16:30h" {{ old('periode', $declaration->periode) == '14:30h-16:30h' ? 'selected' : '' }}>14:30h - 16:30h</option>
            <option value="16:30h-18:30h" {{ old('periode', $declaration->periode) == '16:30h-18:30h' ? 'selected' : '' }}>16:30h - 18:30h</option>
          </select>
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <textarea class="form-control" id="raison" name="raison" rows="4" 
                  placeholder="Veuillez décrire la raison de votre non-disponibilité..." 
                  required maxlength="500">{{ old('raison', $declaration->raison) }}</textarea>
        <div class="form-text"><span id="charCount">{{ strlen(old('raison', $declaration->raison)) }}</span>/500 caractères</div>
      </div>

      <div class="d-grid gap-2 d-md-flex justify-content-md-end">
        <a href="{{ route('enseignant.historique') }}" class="btn btn-secondary me-md-2">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Logique spécifique à l'édition de déclaration
    const typePeriode = document.getElementById('type_periode');
    const periodeField = document.getElementById('periode_field');
    const periodeSelect = document.getElementById('periode');
    const raisonTextarea = document.getElementById('raison');
    const charCount = document.getElementById('charCount');

    // Gestion de l'affichage du champ période
    if (typePeriode) {
    ...
    if (raisonTextarea) {
      raisonTextarea.addEventListener('input', function() {
    ...
    if (editForm) {
      editForm.addEventListener('submit', function(e) {
        ...
        const dateFin = document.getElementById('date_fin').value ? new Date(document.getElementById('date_fin').value) : null;
        ...
        if (typePeriode && typePeriode.value === 'periode' && !periodeSelect.value) {
          e.preventDefault();
